...
function App(): JSX.Element {
  ...
  const handleScroll = () => {
    if (containerRef.current) {
      ...
      const isNearBottom = scrollHeight - scrollTop - clientHeight < SCROLL_MARGIN;
      setIsScrolledUp(!isNearBottom);
    }
  };

  const generateMessageIdentifier = (message: string, username: string) => {
  ...
  useEffect(() => {
    ...
    const handleMessage = (_event: any, data: MessageEventData) => {
      ...
      setMessages(prevMessages => {
        const messageIdentifier = generateMessageIdentifier(data.message, data.username);
        if (data.extra?.id) {
          if (prevMessages.some(message => message.extra?.id === data.extra?.id)) {
            return prevMessages;
          }
        } else {
          if (prevMessages.some(message => generateMessageIdentifier(message.message, message.username) === messageIdentifier)) {
            return prevMessages;
          }
        }
  ...
  useEffect(() => {
    console.log("Entrei", isScrolledUp)
    if (!isScrolledUp) {
      scrollToBottom();
    }
  ...
  return (
    <div className={`relative flex flex-col w-full h-screen rounded-md ${config.appBackgroundColor}`}>
      ...
      <div
        data-messageslength={messages.length}
        className="data-[messageslength=0]:hidden flex-1 flex-col overflow-x-hidden overflow-y-scroll p-2 pb-4 justify-end"
        ref={containerRef}
      >
        {
